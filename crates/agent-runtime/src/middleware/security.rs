// INPUT:  agent_core::middleware, agent_security, agent_types, async_trait, tokio::sync::Mutex
// OUTPUT: SecurityMiddleware
// POS:    Wraps SecurityGuard as async Middleware — blocks tool calls on Deny/NeedHumanApproval.

use std::sync::Arc;

use agent_core::middleware::{Middleware, MiddlewareContext, MiddlewareError};
use agent_security::{SandboxMode, SecurityDecision, SecurityGuard};
use agent_types::{ToolCall, ToolContext};
...
mod tests {
    use super::*;
    use agent_core::middleware::Extensions;
    ...
    async fn blocks_outside_workspace_path() {
        ...
        let tool_ctx = agent_types::EmptyToolContext;
