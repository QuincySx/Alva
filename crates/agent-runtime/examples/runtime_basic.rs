...
//! - Resolving a model via `agent_runtime::model("provider/model_id", &registry)`
...
use agent_core::middleware::{MiddlewareContext, MiddlewareError};
use agent_types::{
...
impl LanguageModel for StubModel {
    async fn complete(
        ...
        Ok(Message {
            ...
            tool_call_id: None,
            usage: Some(agent_types::UsageMetadata {
                input_tokens: 10,
                output_tokens: 8,
                total_tokens: 18,
            }),
...
fn main() {
    ...
    let llm = agent_runtime::model("stub/any-model-id", &registry)
    ...
    rt.block_on(async {
        println!("\n--- Prompting agent ---");
        let user_msg = agent_core::AgentMessage::Standard(Message::user("Hello, agent!"));
        let mut rx = runtime.agent.prompt(vec![user_msg]);

        while let Some(event) = rx.recv().await {
            match &event {
                agent_core::AgentEvent::AgentStart => {
                    println!("[event] AgentStart");
                }
                agent_core::AgentEvent::TurnStart => {
                    println!("[event] TurnStart");
                }
                agent_core::AgentEvent::MessageEnd { message } => {
                    if let agent_core::AgentMessage::Standard(msg) = message {
                        println!("[event] MessageEnd: {}", msg.text_content());
                    }
                }
                agent_core::AgentEvent::AgentEnd { error } => {
