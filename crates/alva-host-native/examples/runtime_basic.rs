...
impl LanguageModel for StubModel {
    async fn complete(
        ...
        Ok(CompletionResponse::from_message(Message {
            ...
            tool_call_id: None,
            usage: Some(alva_kernel_abi::UsageMetadata {
                input_tokens: 10,
                output_tokens: 8,
                total_tokens: 18,
            }),
...
fn main() {
    ...
    rt.block_on(async {
        ...
        while let Ok(event) = event_rx.try_recv() {
            match &event {
                alva_kernel_core::AgentEvent::AgentStart => {
                    println!("[event] AgentStart");
                }
                alva_kernel_core::AgentEvent::TurnStart => {
                    println!("[event] TurnStart");
                }
                alva_kernel_core::AgentEvent::MessageEnd { message } => {
                    if let AgentMessage::Standard(msg) = message {
                        println!("[event] MessageEnd: {}", msg.text_content());
                    }
                }
                alva_kernel_core::AgentEvent::AgentEnd { error } => {
