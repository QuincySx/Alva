...
## 地位
`alva-host-native` 的中间件子模块。存放依赖领域 crate（如 `alva-agent-security`）的中间件实现。这些中间件不适合放在 `alva-kernel-core` 中（core 不应依赖领域 crate），所以放在 runtime 层。

## 逻辑
1. `mod.rs` 声明 `security` 模块并 re-export `SecurityMiddleware`。
2. `security.rs` 将 `SecurityGuard`（来自 `alva-agent-security`）包装为 `alva-kernel-core::middleware::Middleware` trait 实现，在工具调用前执行安全检查：
