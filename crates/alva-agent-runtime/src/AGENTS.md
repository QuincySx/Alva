...
## 逻辑
1. `builder.rs` 提供 `AgentRuntimeBuilder`，通过 Builder 模式配置 model、tools、middleware、hooks 和 bus，最终构建 `AgentRuntime`（包含 Agent + ToolRegistry）；未注入外部 bus 时会自动创建一个默认 bus 并写入 `AgentConfig.bus`。
...
| AgentRuntimeBuilder | builder.rs | Builder 模式构建 AgentRuntime：配置 model、tools、middleware、hooks 与默认/外部 bus |
