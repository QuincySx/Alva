...
1. guest 构造版本化 fetch 或 host escalation 请求并经 ptr/len import 发送给宿主。
2. 宿主返回版本化结果信封；HTTP 成功携带状态/头/body，升级成功携带 stdout/stderr/exit_code，策略/传输失败携带可恢复错误文本。
...
5. body 在 wire 上用 base64 避免 JSON 数字数组放大；两侧共享请求、响应和 JSON/body 大小上限，避免协议漂移与 guest 内存失控。
...
| capability ABI | `src/lib.rs` | fetch / escalation 请求响应与 guest→host 审计事件的版本化、有界 DTO。 |
