...
## 地位
系统的业务核心层。将 alva-types / alva-core / protocol-* 等底层 crate 与具体业务需求结合，提供完整的 Agent 运行时（含 16 种内置工具）、多层安全防护、SQLite 持久化、MCP 工具桥接、技能系统集成和嵌入式运行时环境管理。采用 DDD 分层架构（domain / ports / adapters）。
...
- srow-app 仅依赖 srow-core（通过 Facade 模式），不直接依赖 alva-types/alva-core/alva-graph
