// INPUT:  srow_core::agent::agent_client::{protocol, connection}, tokio::sync::mpsc, std::path
...
use srow_core::agent::agent_client::{
    protocol::{
            ...
            Ok(Some(msg)) => match msg {
                ...
                AcpInboundMessage::SessionUpdate { content, .. } => {
                    for block in content {
                        if let srow_core::agent::agent_client::protocol::content::ContentBlock::Text {
                            text,
                            ..
                        } = block
                        {
                ...
                AcpInboundMessage::TaskComplete { data } => {
                    assert_eq!(
                        data.finish_reason,
                        srow_core::agent::agent_client::protocol::lifecycle::TaskFinishReason::Complete
                    );
...
async fn test_acp_echo_agent_immediate_shutdown() {
    ...
    for _ in 0..20 {
        ...
        if !matches!(
            final_state,
            srow_core::agent::agent_client::connection::processes::ProcessState::Running
        ) {
            break;
        }
    }
    ...
    assert!(
        matches!(
            final_state,
            srow_core::agent::agent_client::connection::processes::ProcessState::Exited
                | srow_core::agent::agent_client::connection::processes::ProcessState::Crashed { .. }
        ),
