...
## 地位
srow-agent 项目的前端 crate，提供基于 GPUI-CE 的原生桌面应用，用户通过该界面与 AI Agent 交互。依赖 `srow-core` 提供引擎能力。
...
- **架构**：Model-View-Bridge 三层结构。
  ...
  - `engine_bridge/`：将 `srow_core::AgentEngine` 适配到 GPUI 异步模型。
