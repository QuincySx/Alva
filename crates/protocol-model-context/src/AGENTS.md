...
## 地位
MCP 协议的独立客户端实现。负责与外部 MCP 服务器的连接管理、工具发现和调用，并通过 McpToolAdapter 将 MCP 工具适配为 agent-base 的 Tool trait，使其可被 agent 引擎直接使用。
...
McpToolAdapter
  └─→ MCP tool → agent_base::Tool (trait object)
...
- McpToolAdapter 将 MCP 的 JSON Schema 映射为 agent-base 的 Tool trait
...
| 工具适配 | `tool_adapter.rs` | McpToolAdapter 将 MCP 工具适配为 agent_base::Tool |
