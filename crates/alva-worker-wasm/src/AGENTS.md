...
`main.rs` 在 WASI 下解析 host 注入的 workspace/task/result/grant args，用 `AgentBuilder` 装配 `ProxyModel`、`CorePlugin`、audit middleware 与 `RunScriptTool`；`run_script.rs` 为每次调用新建 QuickJS runtime，以同步 `WasiFs` 和 fetch 绑定完成批量操作并把脚本错误返回模型。模型、HTTP 与 audit event 分别走独立版本化 JSON import，agent 最终文本写入指定文件或 stdout。
