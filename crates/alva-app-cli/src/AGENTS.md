...
`main.rs` 是参数和模式总路由；`agent_setup.rs` 负责普通 native agent；`bundled_skills.rs` 解包并解析宿主内置 skill；`wasm_sandbox.rs` 负责 wasm-env context 下发、worker sidecar、preopen 映射、host proxy callbacks 及 escalation 的 cwd 翻译/审批/执行；`job_log.rs` 让 native middleware、wasm log import 与 host escalation 共用宿主 JSONL 审计格式；`repl.rs` 对未知斜杠命令做 skill registry 精确点名 fallback；其余模块承载 UI、session 与子命令。
