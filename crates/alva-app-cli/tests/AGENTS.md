# alva-app-cli/tests
> 真实 `alva` binary 的 headless/subcommand/wasm-tier/macOS/Linux os-tier golden 契约。
...
各 golden 文件按用户入口分组；wasm suite 另启动 recording provider 并检查宿主 HTTP 与 guest 文件结果；os suite 在对应平台验证完整 worker 改文件并由 Seatbelt/Landlock 继承到 cargo 子进程。
...
- os suite 不得 ignore；macOS 托管嵌套沙箱不能运行时应报告 `sandbox_apply` 阻断，由无限制 macOS 环境补跑；Linux Landlock 测试由 Linux CI 执行。
...
| OS golden | `os_sandbox_golden.rs` | macOS/Linux `--sandbox os-write` 完整 native worker 修改文件、通过权限门禁并运行 `cargo test --offline`。 |
