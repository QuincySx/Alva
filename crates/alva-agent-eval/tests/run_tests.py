#!/usr/bin/env python3
"""Automated test runner for alva-agent-eval test cases.

Usage:
  1. Start mock:  python3 tests/mock_llm.py &
  2. Start eval:  RUST_LOG="warn,alva_agent_core=debug,alva_llm_provider=debug,alva_agent_runtime=debug,alva_agent_security=debug,alva_agent_tools=info,alva_app_core=info,alva_agent_eval=info" cargo run -p alva-agent-eval &
...
check("D5.1", "含 alva_agent_core", "alva_agent_core" in targets, str(targets))
...
if rec:
    ...
    # Child agent's LLM calls should appear in logs (from alva_agent_core::run)
