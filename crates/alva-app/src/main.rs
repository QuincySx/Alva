// INPUT:  gpui, gpui_component, alva_app (models, theme, views), tracing_subscriber
...
use alva_app::chat::SharedRuntime;
use alva_app::models::{AgentModel, ChatModel, SettingsModel, SettingsModelEvent, WorkspaceModel};
use alva_app::theme::ActiveThemeMode;
use alva_app::views::RootView;
...
fn main() {
    ...
    let (_debug_handle, _view_registry, _action_registry) = {
        ...
        let action_registry = Arc::new(alva_app_debug::ActionRegistry::new());
    ...
    Application::new().run(|cx: &mut App| {
        ...
        #[cfg(debug_assertions)]
        cx.set_global(alva_app::DebugViewRegistry(_view_registry));

        // Store debug ActionRegistry as a GPUI global so components can register actions/state
        #[cfg(debug_assertions)]
        cx.set_global(alva_app::DebugActionRegistry(_action_registry));
