...
//         super::{Middleware, MiddlewareContext, MiddlewareError, MiddlewarePriority}
// OUTPUT: DanglingToolCallMiddleware
// POS:    Scans messages before LLM calls for Assistant ToolUse blocks that have no matching
//         Tool message (ToolResult). Inserts synthetic error ToolResult messages to prevent
//         API errors when resuming interrupted conversations.
...
mod tests {
    ...
    #[tokio::test]
    async fn test_synthetic_inserted_after_assistant_message() {
        let mw = DanglingToolCallMiddleware::new();
        let mut ctx = test_ctx();
        ...
        // Synthetic should be inserted at index 2 (after assistant at index 1)
        assert_eq!(messages.len(), 4);
        assert_eq!(messages[2].role, MessageRole::Tool);
        assert_eq!(messages[2].tool_call_id, Some("tc_1".to_string()));
        // The user message should now be at index 3
        assert_eq!(messages[3].role, MessageRole::User);
    }

    #[tokio::test]
    async fn test_multiple_assistant_messages_with_dangling() {
        let mw = DanglingToolCallMiddleware::new();
        let mut ctx = test_ctx();
