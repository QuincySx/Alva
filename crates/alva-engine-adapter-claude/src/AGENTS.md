...
2. `execute()` 调用 `ensure_bridge_script()` 将编译时内嵌的 bridge JS 脚本写入用户缓存目录（幂等），然后组装 `BridgeConfig` JSON 并通过 `BridgeProcess::spawn()` 启动 Node.js 子进程。
...
6. 支持 Bedrock / Vertex / Azure 云厂商切换，通过环境变量注入子进程。
