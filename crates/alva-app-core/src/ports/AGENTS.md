...
## 地位
在 `alva-app-core` 的六边形架构中，ports 定义驱动端口（Tool、LLMProvider）和被驱动端口（SessionStorage），是 engine 与外部世界的契约边界。
...
| provider | `provider/` | Provider trait、ProviderRegistry、ProviderError（re-export from alva-types） |
