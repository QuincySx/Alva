// INPUT:  std::fs, serde, uuid, chrono, alva_host_native::middleware::CheckpointCallback
// OUTPUT: CheckpointMeta, CheckpointManager, ManagerCheckpointCallback
// POS:    File-based checkpoint store shared by every app shell (CLI + Tauri).
//         Moved up from alva-app-cli so Tauri stops silently running the
//         default-on CheckpointMiddleware with no callback wired.
...
/// The standard `CheckpointCallback` — bridges `CheckpointMiddleware` to a
/// [`CheckpointManager`]. Every app shell should wire this (or a custom
/// callback) via `BaseAgent::set_checkpoint_callback`; without one the
/// default-on middleware silently does nothing.
pub struct ManagerCheckpointCallback {
    manager: CheckpointManager,
}

impl ManagerCheckpointCallback {
    pub fn new(manager: CheckpointManager) -> Self {
        Self { manager }
    }
}

impl alva_host_native::middleware::CheckpointCallback for ManagerCheckpointCallback {
...
mod tests {
    ...
    /// The bridge callback actually lands a checkpoint on disk — wiring
    /// this into an app shell is what turns the default-on
    /// CheckpointMiddleware from a silent no-op into a real safety net.
    #[test]
    fn manager_callback_creates_checkpoints() {
        use alva_host_native::middleware::CheckpointCallback as _;
        ...
        let cb = ManagerCheckpointCallback::new(CheckpointManager::new(workspace));
        cb.create_checkpoint("auto", &[file.clone()]);

        let mgr = CheckpointManager::new(workspace);
        let list = mgr.list();
        assert_eq!(list.len(), 1);
        assert_eq!(list[0].description, "auto");
    }
}
