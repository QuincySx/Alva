// INPUT:  alva_app_core::agent::agent_client::{protocol, connection}, tokio::sync::mpsc, std::path
// OUTPUT: (test functions, no library exports)
// POS:    Integration tests for ACP protocol using echo_agent.py: full lifecycle, immediate shutdown, and state machine transitions.
//! ACP integration tests using echo_agent.py

use std::path::PathBuf;

use alva_app_core::agent::agent_client::{
    protocol::{
            ...
            Ok(Some(msg)) => match msg {
                ...
                AcpInboundMessage::SessionUpdate { content, .. } => {
                    for block in content {
                        if let alva_app_core::agent::agent_client::protocol::content::ContentBlock::Text {
                            text,
                            ..
                        } = block
                        {
                ...
                AcpInboundMessage::TaskComplete { data } => {
                    assert_eq!(
                        data.finish_reason,
                        alva_app_core::agent::agent_client::protocol::lifecycle::TaskFinishReason::Complete
                    );
...
async fn test_acp_echo_agent_immediate_shutdown() {
    ...
    for _ in 0..20 {
        ...
        if !matches!(
            final_state,
            alva_app_core::agent::agent_client::connection::processes::ProcessState::Running
        ) {
            break;
        }
    }
    ...
    assert!(
        matches!(
            final_state,
            alva_app_core::agent::agent_client::connection::processes::ProcessState::Exited
                | alva_app_core::agent::agent_client::connection::processes::ProcessState::Crashed { .. }
        ),
