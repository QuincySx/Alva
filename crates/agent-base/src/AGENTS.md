...
- `Tool` trait 定义工具执行接口，`ToolRegistry` 管理工具集合
...
| 名称 | 文件 | 职责 |
|------|------|------|
| 消息 | `message.rs` | Message、MessageRole、ToolCallData、UsageMetadata |
| 内容块 | `content.rs` | ContentBlock 枚举（text / tool_use / tool_result / image） |
| 工具 | `tool.rs` | Tool trait、ToolCall、ToolResult、ToolRegistry |
