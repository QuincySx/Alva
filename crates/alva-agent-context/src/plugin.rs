// INPUT:  alva_types::AgentMessage, async_trait, crate::sdk::ContextSDK, crate::types (ContextSnapshot, CompressAction, ContextEntry, Injection, IngestAction)
...
pub trait ContextHooks: Send + Sync {
    ...
    async fn bootstrap(&self, sdk: &dyn ContextSDK, agent_id: &str) -> Result<(), ContextError> {
        let _ = (sdk, agent_id); Ok(())
    }

    async fn on_message(&self, sdk: &dyn ContextSDK, agent_id: &str, message: &AgentMessage) -> Vec<Injection> {
        let _ = (sdk, agent_id, message); vec![]
    }

    async fn on_budget_exceeded(&self, sdk: &dyn ContextSDK, agent_id: &str, snapshot: &ContextSnapshot) -> Vec<CompressAction> {
        let _ = (sdk, agent_id, snapshot); vec![CompressAction::SlidingWindow { keep_recent: 20 }]
    }

    async fn assemble(&self, sdk: &dyn ContextSDK, agent_id: &str, entries: Vec<ContextEntry>, token_budget: usize) -> Vec<ContextEntry> {
        let _ = (sdk, agent_id, token_budget); entries
    }

    async fn ingest(&self, sdk: &dyn ContextSDK, agent_id: &str, entry: &ContextEntry) -> IngestAction {
        let _ = (sdk, agent_id, entry); IngestAction::Keep
    }

    async fn after_turn(&self, sdk: &dyn ContextSDK, agent_id: &str) {
        let _ = (sdk, agent_id);
    }
