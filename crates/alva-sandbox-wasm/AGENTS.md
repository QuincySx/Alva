...
1. 调用者把 wasm 模块字节、guest 参数和宿主目录到 guest 挂载点的授权映射交给 `run_module` / `SandboxRunner`。
...
- runner 不依赖 kernel、agent、app 或 host crate；LLM/QuickJS/网络等具体代理策略由调用方通过 per-run import callback 接线。
...
- 运行依赖仅限 wasmtime、wasmtime-wasi 与错误封装库，不依赖任何 Alva workspace crate；具体 LLM trait 接线留在调用方/测试。
