...
## 地位
`alva-agent-graph` 位于单 agent 内核之上，提供多步骤状态图编排能力。它本身不依赖 `alva-kernel-core` 的 run loop，而是通过独立的 graph builder 与 Pregel-style executor 驱动工作流。
...
- `session.rs` 把 compiled graph 与 retry/compaction/checkpoint 配置打包成高层 `GraphRun` 包装器，但执行入口仍然是 `CompiledGraph`。
...
| Crate 配置 | `Cargo.toml` | 依赖声明：alva-kernel-abi、tokio、serde 等 |
