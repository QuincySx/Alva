...
### Task 2.2: 下沉**值类型**（不含 adapter），kernel-abi 改 re-export

> ⚠ 顺序关键：adapter 此刻 **不动**，仍留在 kernel-abi。因为它现在还 `use crate::tool::Tool`（可执行 trait，留在 kernel-abi）。等 Phase 3 把 `encode_tools` 改成吃 `ToolDefinition`、adapter 不再依赖 `Tool` 后，Task 3.3 再把 adapter 整体搬进 wire。本 task 只搬纯值类型。

**Files:**
- Move into `crates/alva-llm-wire/src/`: `base/{content,message,stream}.rs`、`model/config.rs`（→`config.rs`）、`tool/content_payload.rs`（→`tool_payload.rs`）、`tool/types.rs` 中的 `ToolDefinition`（→`tool_def.rs`，只搬这一个 struct）
- Modify: `crates/alva-llm-wire/Cargo.toml`（加 wasm target 依赖块）
- Modify: `crates/alva-kernel-abi/Cargo.toml`（加 `alva-llm-wire = { path = "../alva-llm-wire" }`）
- Modify: `crates/alva-kernel-abi/src/{lib.rs, base/mod.rs, base/content.rs, base/message.rs, base/stream.rs, model/mod.rs, tool/mod.rs, tool/types.rs}`（改为 re-export）
- **不动**：`crates/alva-kernel-abi/src/adapter/*`（仍引用 `crate::base::message::*`，经 re-export 解析；仍用 `crate::tool::Tool`）

- [ ] **Step 1: wire Cargo.toml 加 wasm target 依赖块**

对照 `alva-kernel-abi/Cargo.toml` 里 wasm 的处理，加：
```toml
[target.'cfg(target_family = "wasm")'.dependencies]
uuid = { version = "1", features = ["v4", "js"] }
```
（chrono 的 `clock` 在 wasm 下用 `wasmbind`——若 `alva-kernel-abi` 是靠 `wasmbind` feature，则这里 chrono 也加 `features=["clock","wasmbind"]`；以 kernel-abi 现写法为准。）

- [ ] **Step 2: 物理迁移值类型 + wire lib.rs 导出**

把上述**值类型**文件移入 wire crate：
```rust
// alva-llm-wire/src/lib.rs
pub mod content;
pub mod message;
pub mod stream;
...
```
crate 内互引用路径改成 wire 路径：`content.rs` 引 `crate::tool_payload::ToolContent`（不再 `crate::tool::content_payload`）；`message.rs` 引 `crate::content::ContentBlock`；`stream.rs` 引 `crate::message::UsageMetadata`。`ToolDefinition` 从 `tool/types.rs` 只抽这一个 struct（`Tool` trait / `ToolRegistry` / `ToolCall` 等留 kernel-abi）。

- [ ] **Step 3: kernel-abi re-export（双路径，零下游改动）**

- `base/content.rs` → 整文件替换为 `pub use alva_llm_wire::content::*;`（message/stream 同理）。`base/mod.rs` 保留 `pub mod content; pub mod message; pub mod stream;`（现为 shim）。
- `model/mod.rs`：`pub use alva_llm_wire::config::{ModelConfig, ReasoningEffort};`（保 `alva_kernel_abi::model::ModelConfig` 模块路径）。`model/config.rs` 删除。
- `tool/types.rs`：删 `ToolDefinition` 定义，改 `pub use alva_llm_wire::tool_def::ToolDefinition;`（`Tool::definition()` 仍返回它）。`tool/content_payload.rs` 删除，`tool/mod.rs` 改 `pub use alva_llm_wire::{ProgressEvent, ToolContent, ToolOutput, ToolDefinition};`（保 `crate::tool::content_payload::*` 旧路径——若有用此路径者，加一个 `pub use alva_llm_wire::tool_payload as content_payload;` shim）。
- `lib.rs` 顶层扁平 re-export 逐一对照现清单改成 `pub use alva_llm_wire::{...}`。
- **adapter 不动**：`crate::base::message::Message` 等经 re-export 仍解析。

- [ ] **Step 4: wire 自测 + 全工作区**

Run: `cargo test -p alva-llm-wire -- --nocolor`（迁移来的单测随之绿）
Run: `cargo check --workspace`
Expected: 绿（所有旧 import 路径经 re-export 不变；adapter 仍在 kernel-abi、仍编译）。

- [ ] **Step 5: wasm 门禁 + ci 名单**

Run: `cargo check -p alva-llm-wire --target wasm32-unknown-unknown` → 绿。
把 `alva-llm-wire` 加入 `scripts/ci-check-deps.sh` 的 wasm-clean 名单 + "无 app/host 依赖" 规则。

- [ ] **Step 6: Commit**
```bash
git add -A
git commit -m "refactor: extract alva-llm-wire value types (down-sink content/message/stream/config/payload/tooldef)

adapter 暂留 kernel-abi（仍依赖 Tool trait），Phase 3 解耦后再搬。
kernel-abi 全面 re-export 保旧路径（扁平 + 模块）。纯搬运，零行为变化。"
```

---

## Phase 3 — `encode_tools` 解耦 `Tool` trait，然后搬 adapter 进 wire

### Task 3.1: trait 改名 + `encode_tools` 签名换 `ToolDefinition`（adapter 仍在 kernel-abi）

**Files:**
- Modify: `crates/alva-kernel-abi/src/adapter/mod.rs`（trait 定义，现 `ToolAdapter`→`ProtocolAdapter`，`encode_tools(&[&dyn Tool])`→`&[ToolDefinition]`）
- Modify: `crates/alva-kernel-abi/src/adapter/{anthropic,openai_chat,openai_responses,gemini}.rs` 的 `encode_tools`（用 `td.name`/`td.description`/`td.parameters` 取代 `t.name()`/...）
- Modify: 各 adapter in-file 测试（构造 `ToolDefinition` 取代 `&dyn Tool`/MockTool）
- Modify: `crates/alva-kernel-abi/src/lib.rs`（`ToolAdapter` deprecated 别名指向 `ProtocolAdapter`）
...
- [ ] **Step 4: 兼容别名（adapter 此刻在 kernel-abi）**

在 `crates/alva-kernel-abi/src/adapter/mod.rs` 定义 `pub trait ProtocolAdapter` 后，加 trait 别名 re-export 让旧名仍可用：`pub use self::ProtocolAdapter as ToolAdapter;`（这样 `alva_kernel_abi::adapter::ToolAdapter` 仍解析到该 trait，下游 provider 的 `use ...::ToolAdapter` 不动）。删 `adapter/mod.rs` 里已不需要的 `use crate::tool::Tool;`，加 `use crate::tool::ToolDefinition;`（kernel-abi 经 Task 2.2 已 re-export 它）。

- [ ] **Step 5: 测**

Run: `cargo test -p alva-kernel-abi -- --nocolor` → PASS（adapter 仍在 kernel-abi）。

- [ ] **Step 6: Commit**
```bash
git add crates/alva-kernel-abi/
git commit -m "refactor(kernel-abi): rename ToolAdapter→ProtocolAdapter, encode_tools takes ToolDefinition

与可执行 Tool trait 解耦：encode_tools 只需 name/desc/parameters。保留 ToolAdapter 别名。"
```
...
### Task 3.3: 把 adapter 模块搬进 wire（此刻已 Tool-free）

> 现在 adapter 不再依赖 `crate::tool::Tool`（Task 3.1 已去除），可以安全搬进 wire crate。

**Files:**
- Move: `crates/alva-kernel-abi/src/adapter/*` → `crates/alva-llm-wire/src/adapter/*`
- Modify: `crates/alva-llm-wire/src/lib.rs`（加 `pub mod adapter;`）
- Modify: `crates/alva-kernel-abi/src/lib.rs`（删本地 `mod adapter;`，改 `pub use alva_llm_wire::adapter;`）

- [ ] **Step 1: 物理迁移 + 改 crate 内路径**

把整个 `adapter/` 目录移入 wire。adapter 内所有 `use crate::base::message::*` / `crate::base::stream::*` / `crate::base::content::*` 改成 wire 路径 `crate::message::*` / `crate::stream::*` / `crate::content::*`；`crate::tool::ToolDefinition` → `crate::tool_def::ToolDefinition`；`super::common::*` 等同目录引用不变。`adapter/mod.rs` 顶部的 `pub use self::ProtocolAdapter as ToolAdapter;` 一并带过来。

- [ ] **Step 2: wire 导出 + kernel-abi re-export**

`alva-llm-wire/src/lib.rs` 加 `pub mod adapter;`。`alva-kernel-abi/src/lib.rs`：删 `mod adapter;`（本地已无），改 `pub use alva_llm_wire::adapter;`——这样 `alva_kernel_abi::adapter::{ProtocolAdapter, ToolAdapter, EncodedMessages, DecodedResponse, StreamDecodeState, AdapterError, anthropic::*, ...}` 全部经 re-export 不变。

- [ ] **Step 3: wire 自测 + 全工作区**

Run: `cargo test -p alva-llm-wire -- --nocolor`（adapter 单测随迁移绿）
Run: `cargo check --workspace`
Expected: 绿（provider 的 `use alva_kernel_abi::adapter::{ToolAdapter, ...}` + 具体 adapter 路径经 re-export 全部解析）。
...
```bash
git add -A
git commit -m "refactor: move adapter module into alva-llm-wire (now Tool-free)

adapter 已不依赖可执行 Tool trait，搬进 wire crate；kernel-abi re-export adapter
模块保旧路径。至此 wire 自洽：值类型 + ProtocolAdapter 全在一处，可独立发布。"
```
