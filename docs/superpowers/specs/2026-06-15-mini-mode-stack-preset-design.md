...
> 状态:❌ 已弃用(2026-06-15)。用户判定 Preset/StackProfile 框架本身就是"摊子铺太大"的过度设计。
> 改为:迷你模式 = 纯 `BaseAgentBuilder`(默认只自动装 memory/security/system_context,无额外组件),开发时**组件增量手动加回**,不做统一预设系统。本文档仅作为"为何不这么做"的记录保留。
> 日期:2026-06-15
