...
| `alva-worker-wasm` | WASIp1 agent worker：用 SDK `AgentBuilder` + WASI 文件工具在 guest 内运行真 agent loop，经结构化阻塞式 ptr/len import 代理宿主 LLM stream，并把最终回答写入 `/work/result.txt`。 |
