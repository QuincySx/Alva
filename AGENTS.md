...
| `alva-sandbox-abi` | L0 | Sandbox guest/host 共享的纯 serde 能力 ABI；当前定义版本化、限额化 fetch 请求/响应/错误信封。零 workspace 依赖、wasm-clean，不拥有网络或授权策略。 |
...
| `alva-sandbox-wasm` | Native-only WASIp1 模块宿主 runner。每次调用新建 wasmtime `Engine` / `Store` / `WasiCtx`，把 job 授权目录作为 preopen 映射进 guest，并捕获退出码与 stdout/stderr；提供 L0 DTO 驱动的 ptr/len LLM import 桥及宿主强制域名白名单、逐跳重定向校验的 fetch import。 |
| `alva-worker-wasm` | WASIp1 agent worker：用 SDK `AgentBuilder` + WASI 文件工具在 guest 内运行真 agent loop，经版本化、限额化 ptr/len import 代理宿主 LLM stream 与同步 fetch；workspace、任务、授权 guest path 与结果通道全部由 WASI args 注入。 |
...
| crate | 职责 |
|------|------|
| `alva-app-cli` | CLI 入口（`pi` 风格的交互式终端 + REPL）；`-p --sandbox wasm --grant <dir>` 生产路径在宿主保留 provider/key，并用 sidecar worker 跑 WASIp1 agent loop。 |
