...
> kernel 和 agent-core 不接受功能扩展，所有可选行为都是 `Tool` / `Plugin` / `Middleware`。
...
2. **Plugin**（`alva-agent-core::extension::Plugin`）—— 一组功能的封装，可注册 tool、
   middleware、bus 能力、system prompt 片段与命令元数据。**首选扩展方式**。
3. **Middleware**（`alva-kernel-core::Middleware`）—— 直接挂到 agent loop 的洋葱
   中间件。一般通过 Plugin 间接注册，单点中间件也可以直接 `.middleware()`。

### 默认替换契约

`BaseAgent` 自带几个默认 Plugin（`MemoryPlugin::default()`、
`SecurityPlugin::for_workspace()`、`SystemContextPlugin::new()`）。要替换其中任何一个，**注册一个 `name()` 相同的
Plugin**——`BaseAgentBuilder::build()` 检测到同名时跳过默认。

```rust
// 替换默认 memory 后端
struct PostgresMemoryPlugin { /* ... */ }
impl Plugin for PostgresMemoryPlugin {
    fn name(&self) -> &str { "memory" }   // ← 同名，默认被跳过
    async fn register(&self, r: &Registrar) {
        r.provide(Arc::new(self.service.clone()));
    }
}

BaseAgent::builder()
    .workspace("/path")
    .plugin(Box::new(PostgresMemoryPlugin::new(...)))
    .build(model).await?;
```
...
当前由 `alva-app-core::EvaluationPlugin` 在内部使用。
...
> - **`alva-agent-{context,memory,security,graph}`** —— 能力库，被 plugin 消费。
> - **`alva-agent-extension-builtin`** —— 内置 tool 实现 + 默认 plugin 包装。
...
| `alva-sandbox-abi` | L0 | Sandbox guest/host 共享的纯 serde 能力 ABI；当前定义版本化、限额化 fetch 请求/响应/错误信封与单向 audit event。零 workspace 依赖、wasm-clean，不拥有网络、授权或日志持久化策略。 |
...
| `alva-agent-core` | L2.5 SDK 装配 | **Plugin 系统的合约 + 装配入口**。`Plugin` trait（`name` / `description` / `register` / `finalize`）+ `Registrar`（单一跨层装配句柄：`tool` / `tools` / `phase`（metadata-only）/ `phase_handler`（executable）/ `middleware` / `provide` / `system_prompt` / `command` + `bus` / `bus_writer` / `workspace` 只读访问）+ `PhaseContribution` / `PhaseHandler`（目标 timeline 来自 `alva_kernel_abi::{Phase, PhaseEffect}`，agent-core 只拥有 `PhaseOrder` 与装配/编译逻辑）+ `LateContext`（`finalize` 晚期接线上下文）+ `PluginHost`（运行期 middleware/command/phase contribution 容器）+ `AgentAssemblySnapshot` / `PluginAssemblySnapshot`（装配可观测性）。事件层已删除（plugin 只产出 phase/middleware/tool/bus 能力，不再有 `on` / `ExtensionEvent` / `ExtensionBridgeMiddleware`）。`Agent` + `AgentBuilder`：纯 SDK 级 agent 装配（无任何 harness 决策），第三方搭自己 harness 就从这里入。`MockToolFs` 测试替身。 |

### L3：能力库（被 plugin 消费的具体实现）

| crate | 一句话 | 关键内容 |
|------|------|------|
| `alva-agent-context` | 控制 prompt + 多 agent 协作 | **核心 context 插件系统**：`ContextHooks` 7 钩子生命周期 / `ContextStore` 四层容器（AlwaysPresent / OnDemand / RuntimeInject / Memory）/ `ContextHandle` SDK / `RulesContextHooks` / `DefaultContextHooks`（LLM 回调 + 确定性 fallback）/ `compact.rs` + `auto_compact.rs` / `default_context_system()`。**`middleware.rs`** `CompactionMiddleware`（hook-driven 路径并存）。**`scope/` 子模块**：`Blackboard`（共享消息板 / @mention 协作）+ `BoardMessage` / `MessageKind` / `TaskPhase` + `AgentProfile` + `BlackboardPlugin` + `BoardRegistry`（按 `SpawnScope` 隔离）+ `SessionTracker`（spawn 树父子关系）+ `SpawnScopeImpl`。运行期由 `kernel-core::ContextRuntime` 在 prepare-LLM/on_message/after_turn/dispose 阶段协调。 |
...
| crate | 职责 |
|------|------|
| `alva-agent-extension-builtin` | **内置 tool + 默认 Plugin wrapper**。`tools/` 目录 40+ 个 `Tool` 实现，按 feature 分组（`core` = file_io + shell + interaction + plan primitives；`utility` = config / skill / tool_search / sleep；`web` = internet_search + read_url；`notebook` / `worktree` / `team` / `task` / `schedule` / `browser`）。`tool_presets::*` 函数把工具按组打包。`wrappers/` 目录的 `Plugin` 包装：`CorePlugin` / `ShellPlugin` / `InteractionPlugin` / `TaskPlugin` / `TeamPlugin` / `PlanningPlugin` / `UtilityPlugin` / `WebPlugin` / `BrowserPlugin` / `SystemContextPlugin` —— 这些是 tool 组的 `Plugin` 形式。**`MemoryPlugin::default()`**（默认用 `InMemoryBackend`，可被同名 plugin 替换）。**`SecurityPlugin::for_workspace()`**（默认 sandbox 中间件）。纯中间件壳（旧 `LoopDetectionExtension` / `DanglingToolCallExtension` / `ToolTimeoutExtension` / `CompactionExtension` / `CheckpointExtension`）已删除——这些单点 middleware 现在直接 `.middleware()` 注册，不再包 Plugin 壳。`LocalToolFs`：本地 OS `ToolFs` 适配器，native-only；`WasiFs`：WASI 同步文件系统适配器（无子进程）。`register_builtin_tools(registry)` legacy shim。 |
| `alva-app-extension-browser` | **重依赖外挂**：基于 `chromiumoxide` 的浏览器自动化（CDP），独立 crate 隔离 mio 依赖，wasm 不可用。 |
| `alva-app-extension-memory` | **重依赖外挂**：基于 `rusqlite` + 捆绑 SQLite 的持久化 memory backend (`MemorySqlite`) + 工作区扫描 + LLM 提取。**不是默认 backend**——想要持久化的用户显式注册一个同名 `Plugin` 来替换 `MemoryPlugin::default()`。 |
| `alva-app-extension-loader` | **第三方子进程插件加载器**：实现 AEP（Alva Extension Protocol，JSON-RPC 2.0 over stdio），把 Python / JS / 其它语言写的插件作为子进程加载成标准 `Plugin`。`SubprocessLoaderPlugin` 扫描 `~/.config/alva/extensions/` + `<workspace>/.alva/extensions/`，每个插件用远端代理桥接为本地 Plugin。Native-only（wasm 下编译为空 lib）。插件作者不用写 Rust——配套 SDK 在 `sdk/python`（`alva_sdk`）/ `sdk/js`（规划中）。 |
...
| `alva-sandbox-wasm` | Native-only WASIp1 模块宿主 runner。每次调用新建 wasmtime `Engine` / `Store` / `WasiCtx`，把 job 授权目录作为 preopen 映射进 guest，并捕获退出码与 stdout/stderr；提供 L0 DTO 驱动的 ptr/len LLM import 桥、宿主强制域名白名单/逐跳重定向校验的 fetch import，以及不持有落盘策略的有界 audit event import。 |
| `alva-worker-wasm` | WASIp1 agent worker：用 SDK `AgentBuilder` + WASI 文件工具在 guest 内运行真 agent loop，经版本化、限额化 ptr/len import 代理宿主 LLM stream、同步 fetch 与单向 audit event；workspace、任务、授权 guest path 与结果通道全部由 WASI args 注入，宿主 job 日志路径不进入 guest。 |
...
| crate | 职责 |
|------|------|
| `alva-app-cli` | CLI 入口（`pi` 风格的交互式终端 + REPL）；`-p --sandbox wasm --grant <dir>` 生产路径在宿主保留 provider/key，并用 sidecar worker 跑 WASIp1 agent loop；jobs 由宿主以 native middleware / wasm audit import 实时追加统一 `tools.jsonl`。 |
...
use alva_agent_extension_builtin::wrappers::{CorePlugin, ShellPlugin};

let agent = Agent::builder()
    .model(my_model)
    .plugin(Box::new(CorePlugin))
    .plugin(Box::new(MyCustomPlugin))
    .system_prompt("...")
    .build()
    .await?;
...
let agent = BaseAgent::builder()
    ...
    // 默认自带 MemoryPlugin (InMemoryBackend) + SecurityPlugin + SystemContextPlugin
    // 想换 memory？.plugin(Box::new(MyMemoryPlugin))，name() 撞车自动跳过默认
    .build(my_model)
    .await?;
```
...
| `Cargo.toml` | Rust workspace，管理 35 个 crate。 |
