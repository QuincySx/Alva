...
| `alva-agent-core` | L2.5 SDK 装配 | **扩展系统的合约 + 装配入口**。`Extension` trait（`name` / `tools` / `activate` / `configure` / `finalize`）+ `HostAPI`（`middleware` / `on` / `on_as` / `register_command` / `append_system_prompt` / `shutdown`）+ `ExtensionHost` / `ExtensionContext` / `FinalizeContext` / `ExtensionEvent` + `ExtensionBridgeMiddleware`。`Agent` + `AgentBuilder`：纯 SDK 级 agent 装配（无任何 harness 决策），第三方搭自己 harness 就从这里入。`MockToolFs` 测试替身。 |
