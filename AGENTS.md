...
> Rust 实现的分层架构 AI Agent 平台：alva-types（基础 trait）→ alva-agent-core（循环引擎 + 异步 Middleware）→ alva-agent-tools/security/memory（独立功能 crate）→ alva-agent-runtime（Builder API）→ srow-core（Facade）→ srow-app（GPUI 桌面应用）
